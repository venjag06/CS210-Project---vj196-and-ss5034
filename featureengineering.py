...
# Load housing and cities from DB
conn = sqlite3.connect(DB_PATH)
...
# Merge on region + state (inner join)
# Inner join keeps only listings where we have
# matching Census demographic data for that city.
# ─────────────────────────────────────────────

# Check if 'merged' table already exists in DB to avoid re-merging and re-engineering features on every run
if table_exists(DB_PATH, "merged"):
   ...
   # time for feature engineering 
    ...
    # Price per square foot — normalizes price by size so the model can compare cheap-but-small vs expensive-but-large
    merged["price_per_sqft"] = merged["price"] / merged["sqfeet"]

    # Average rent for that region — tells us the "going rate" for a city, useful for detecting over/under-pricing
    merged["avg_rent_by_region"] = (
        merged.groupby(["region", "state"])["price"].transform("mean")
    )

    # Unemployment rate — converts raw unemployed count to a percentage of population so cities are comparable
    merged["unemployment_rate"] = (merged["unemployed_count"] / merged["population"]) * 100

    # Bachelor's degree rate — converts raw bachelor's degree count to a percentage of population so cities are comparable
    merged["bachelors_rate"] = (merged["bachelors_count"] / merged["population"]) * 100

    # rouding to 4 decimal places for better readability 
    for col in ["price_per_sqft", "avg_rent_by_region", "unemployment_rate", "bachelors_rate"]:
        merged[col] = merged[col].round(4)

    # Drop rows where engineered features couldn't be computed or had invalid values
...
"""
# Quick sanity check for purely debugging purposes
